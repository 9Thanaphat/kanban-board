...
const Card = ({ data, setCards, id, index, isChecked }) => {
        ...
        <Draggable draggableId={String(id)} index={index}>
            {(provided) => (
                ...
                >
                    ...
                    <span className={`text-sm wrap-break-words flex-1 ${isChecked ? 'line-through text-gray-500' : ''}`}>
                        {data}
                    </span>
                    <button onClick={removeCard} className='opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white/10 p-0.5 rounded text-[10px]'>
                        X
                    </button>
                </div>
            )}
